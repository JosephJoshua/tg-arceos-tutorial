...
impl DirNode {
    ...
    /// Renames (moves) an entry within this directory from `src_name` to `dst_name`.
    ...
    /// Navigate from this directory node to the directory at the given path.
    fn navigate_to(&self, path: &str) -> VfsResult<VfsNodeRef> {
        let path = path.trim_start_matches('/');
        if path.is_empty() || path == "." {
            // Return self - but we need a VfsNodeRef. Use the weak self pointer.
            return self
                .this
                .upgrade()
                .map(|arc| arc as VfsNodeRef)
...
/// Split a path into (parent_directory_path, filename).
/// e.g. "/tmp/f1" -> ("tmp", "f1"), "f1" -> ("", "f1")
